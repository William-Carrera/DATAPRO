...
public class WeightedAdjMatrixGraph implements WeightedGraph {
	...
	public static class WAMGBuilder {
		...
		public void connect(int u, int v, int weight) {
			if (graph.edges[u][v] == Integer.MAX_VALUE) {
		...
		public void connectUndirected(int u, int v, int weight) {
			if (graph.edges[u][v] == Integer.MAX_VALUE) {
				graph.edges[u][v] = weight;
	...
	public Iterable<Integer> adjacents(final int v) {
		int j = 0;
		while (j < edges.length && edges[v][j] == Integer.MAX_VALUE)
			j++;
		final int finalJ = j;
			...
			public Iterator<Integer> iterator() {
				return new Iterator<Integer>() {
					...
					public Integer next() {
						...
						do
							i++;
						while (i < edges.length && edges[v][i] == Integer.MAX_VALUE);
						return toReturn;
					}
	...
	public boolean adjacent(int u, int v) {
		return edges[u][v] != Integer.MAX_VALUE;
	}
	...
	public Iterable<WeightedEdge> edges() {
		...
		class EdgeIt implements Iterator<WeightedEdge> {
			...
			EdgeIt() {
				i = 0;
				j = 0;
				while ((i < edges.length && j < edges.length) && edges[i][j] == Integer.MAX_VALUE)
					advance();
			}

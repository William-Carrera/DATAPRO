...
public class ArrayForestDisjointSet implements DisjointSet {
    ...
    public Iterable<Integer> findAll(int p) {
        ...
        return new Iterable<Integer>() {
            public Iterator<Integer> iterator() {
                int i = 0;
                while (i < parents.length && find(i) != leader) i++;
                final int finalI = i;
                
                return new Iterator<Integer>() {
    
                    // The next leader to return, if any. Invariant:
                    // (a) All elements in range [0, next) that are in p's
                    //     set have been returned.
                    // (b) next == parents.length iff all elements in p's set
                    //     have been returned
                    int next = finalI;
                    
                    public boolean hasNext() {
                        return next < parents.length;
                    }
    
                    public Integer next() {
                        int toReturn = next;
                        do next++;
                        while (next < parents.length && find(next) != leader);
                        return toReturn;
                    }
    ...
    public Iterator<Integer> iterator() {
        int i = 0;
        while (i < parents.length && parents[i] != i) i++;
        final int firstLeader = i;
        return new Iterator<Integer>() {
            
            // The next leader to return. Invariant:
            // (a) All leaders in range [0, nextLeader) have been returned
            // (b) nextLeader == n or parents[nextLeader] = nextLeader
            int nextLeader = firstLeader;

            public boolean hasNext() {
                return nextLeader < parents.length;
            }
            public Integer next() {
                if (! hasNext()) throw new NoSuchElementException();
                else {
                    int toReturn = nextLeader;
                    do nextLeader++; 
                    while (nextLeader < parents.length 
                            && parents[nextLeader] != nextLeader);
                    return toReturn;
                }
            }
